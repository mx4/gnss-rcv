...
| **Sustained nav bit-sync on marginal recordings** | ✅ Improved (sync), open (deep). The brittle LNAV recovery is fixed (`gps_lnav.rs`): bit sync now rides out brief weak-bit runs (`WEAK_BIT_LIMIT`, keeping the 300-bit alignment) and a frame-sync slip keeps bit sync — so the 3 consecutive clean subframes an ephemeris needs line up. On the **FGI 2023 recording** GPS went from 3 ephemerides / no fix → a **fix** (54 subframes, 1 lock loss; matches that capture's Galileo fix to ~30 m). gpssim unchanged. Still open for **deeply marginal** captures (SJTU 4-bit/25 MHz, HackRF). **Root-caused further (2026-06-12)**: on SJTU the losses are a *deterministic receiver-internal sawtooth* — lock at 50 dB-Hz, C/N0 decays ~1 dB/s as the prompt slides off the code peak at ~0.05 chips/s, divergence guard fires at ~30 dB-Hz (coroner, not killer), instant re-lock at 50, repeat every ~23 s. GEOs (true Doppler ≈ 0) hold 55 s unbroken with 34 CRC-valid SBAS messages, so the walk-off scales with true satellite Doppler → suspect the carrier-aiding↔DLL interplay mis-scaled for this front end (SX3 4-bit, IF 6.25 MHz, ~+1302 Hz LO offset per div_ema): IF sign/spectral inversion or clock-vs-mixer LO attribution in the aiding term, with the E−L discriminator apparently near-blind to the slide. Next probes: (a) E/P/L correlator history across one death cycle (-p, single SV); (b) conjugate-input spectral-inversion test; (c) wider SP_CORR. |
