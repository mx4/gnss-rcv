...
first fix re-pinned 41.274820, 1.987562 — moved ~1 m when the DLL became a
PI loop (the rate-trim integrator changes every code correction, hence the
bit path); the previous pin 41.274817, 1.987575 was itself a deterministic
ULP-path shift at the 4-SV epoch. Exact-truth suite unchanged (gpssim fix
1.6 m, σ 2.5–3.0 m). All gates PASS.
...
| `sjtu` | `-t 4bit --fs 25000000 --fi 6250000` | ✅ fix 31.026, 121.440 (Shanghai, SJTU Minhang); 6 ephemerides, 60 s unbroken locks since the DLL rate-trim integrator (the front end's ~850 ns/s clock skew used to walk every MEO off the peak in ~23 s); σ still tens of m — no iono corrections and the Doppler-proportional `dll_lag` term mis-calibrates at 25 Msps (open) |
