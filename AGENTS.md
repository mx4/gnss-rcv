...
| **Combined GPS + Galileo fix** | Open; **solver side de-risked**. The two-pass merge experiment (`combined_gps_galileo_fix_from_two_passes`, receiver.rs, ION LimeSDR) proves gnss-rtk solves a mixed 15-SV GPST+GST pool with no special config (291 m vs the ~100 m-precise site; leftover inter-system bias ~40 m / ~130 ns), and found the two real blockers: (1) the **GPS week rollover** (next row) puts pre-2019 captures 1024 weeks off the GST timeline — fatal for a merge, invisible within one constellation; (2) the LNAV and I/NAV **TOW→phase anchors disagree by exactly 1.8400 s** (= 2.0 s page minus 0.16 s/8 LNAV bits — the two decode latencies). Which side carries the absolute offset is unresolved (GPS is synth-validated end-to-end; a Galileo GeoFeed would pin the I/NAV convention absolutely). Remaining for live dual decode: the receiver-side multi-signal stepping (1 ms steps, channels accumulate to their own period). **Best live test data: `tuni2025`** (Tampere 2025, 50 MHz) carries both — 8 Galileo E1B SVs and 16 GPS L1CA SVs, each fixing independently at 61.450, 23.856 — so it's the ideal modern, rollover-free capture to validate a real dual-constellation solve. |
...
| **Galileo OSNMA** | ✅ wired (automatic on any E1B run): I/NAV → 40-bit field (`page[132..172]`) → one shared verifier over the [`galileo-osnma`](https://github.com/daniestevez/galileo-osnma) crate ([`osnma.rs`](src/osnma.rs)), GST per page from the word-5 anchor. On the FGI 2023 recording the **DSM-PKR public key verifies against the GSC Merkle root** (decode byte-perfect); full nav-data auth pends a complete DSM-KROOT (absent from that PKR-dominated capture). 2023/2024/2025 trust anchors built in, auto-selected by decoded GST week. **Next: a live full-auth test on `tuni2025`** (clean, wideband 2025; GST week 1326 auto-selects the **2024 PKID-1** anchor — *not* 2025 PKID-2, which only applies from week 1372 / 2025-12-10) — its 8 E1B SVs over ~150 s are the best shot at reassembling a complete DSM-KROOT for the first live nav-data authentication. Full write-up: [docs/osnma.md](docs/osnma.md). |
